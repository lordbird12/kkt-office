...
export class ListComponent implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        // // If language is stored in localStorage, set it as the active language
        // if (storedLang) {
    ...
    editElement(element: any) {
        this._router.navigate(['admin/banjupan/edit/' + element]);
    }
    addElement() {
        this._router.navigate(['admin/banjupan/form']);
    }
    excelExport() {
        window.open(
            'https://laongherbalgroup.com/api/public/api/export_product/2'
        );
    }
    hiddenEdit() {
        const getpermission = JSON.parse(localStorage.getItem('permission'));
        const menu = getpermission.find((e) => e.menu_id === 12);
        return menu.edit === 0;
    }
    hiddenDelete() {
        const getpermission = JSON.parse(localStorage.getItem('permission'));
    ...
    loadTable(): void {
        ...
        this.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                dataTablesParameters.type = 'Raw';
                dataTablesParameters.category = '2';
                that._service
